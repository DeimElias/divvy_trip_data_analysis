...
We have now finished our data cleaning process. Now that our data is ready, let's continue with our analysis.
...
df |>
  ...
  geom_col(color = "#000000", fill = c("#B7FA8A","#06CEFD", "#B7FA8A","#06CEFD", "#B7FA8A","#06CEFD")) +
...
df |>
  select(started_at, ride_duration) |>
  group_by(month(started_at), mday(started_at)) |>
  mutate(avg_ride_duration = mean(ride_duration)) |>
  ungroup() |>
  select(started_at, avg_ride_duration) |>
  ggplot() + aes(x = mday(started_at), y = max(avg_ride_duration)) + geom_col(color = "#000000", fill = "#06CEFD") + global_theme() +
  facet_wrap(vars(month(started_at, label = TRUE, abbr = FALSE))) 
df
