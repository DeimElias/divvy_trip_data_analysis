...
As we can see, there is no key colums, which is not a problem, since each row is unique. 
We only have 1027 NA. We can drop them, since they represent less than a .1% of the total data. \
...
```
We have multiple greographical values for some stations. Altought this could be intented (to have precise data on where was the bycicle taken/returned), we are not interested in those especifics, lets fix them.
# Data cleaning
```{r}
df <- df %>%
  drop_na()
