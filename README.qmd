...
global_theme <-  function() {
  theme_linedraw() +
  theme(axis.text.y = element_text(colour = 'white', face = 'bold'),
...
df |>
  ...
  ggplot() + aes(x=most_distant) + scale_x_log10() + global_theme() +
  geom_histogram(bins=100, color = "#07D0FF", fill = "#06CEFD") +
  labs(x = "Distance (in meters) to station (calculated by mean)", y = "Number of rides started at given distance", title = "Distribution of start of ride distance to station (calulated by mean)", subtitle = "Station Flied Museum
    (only rides that start at distance greater than 15m")
