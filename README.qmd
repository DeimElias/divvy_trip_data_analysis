...
colSums(is.na(df))
```
The other data that is missing is naming and id info from the stations. We can check if we can fill
the missing data with other entries that have the same geographical information.
```{r}
df |>
...
(to have precise data on where was the bycicle taken/returned), we are not interested in those especifics, we should also fix this.
